...
public class VisitHistory extends HttpServlet {
	...
	/*protected void doDelete(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (input.has("user_id") && input.has("visited")) {
				...
				List<String> visitedRestuarants = new ArrayList<>();
				for (int i = 0; i < array.length(); i++) {
					visitedRestuarants.add(array.getString(i));
				}
				connection.UnsetVisitedRestaurants(userId, visitedRestuarants);
			}
	...
	/*protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameterMap().containsKey("user_id")) {
			...
			Set<String> visitedRestuarants = connection.getVisitedRestaurants(userId);
			array = new JSONArray(visitedRestuarants);
